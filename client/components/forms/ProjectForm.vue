...
<template>
	<BaseCard>
			...
			<v-col v-for="work in availableWorks" :key="work._id" cols="12" md="4">
				<base-card outlined>
					...
					<base-btn
						...
						>Unselect</base-btn
					>
					<base-btn v-else color="success" small @click="selectWorkId(work._id)"
						>Select</base-btn
					>
				</base-card>
			</v-col>
		...
		<BaseBtn v-if="!props.project" color="primary" block @click="post()"
			>Upload</BaseBtn
		>
		<BaseBtn v-if="props.project" color="primary" block @click="patch()"
			>Update</BaseBtn
		>
