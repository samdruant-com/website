...
<template>
  <base-sidebar
    ...
    @close="sidebarStore.visible = false"
  >
    <!-- tailwind list of centered links -->
    <div class="flex flex-col h-4/5 gap-4 items-center place-content-center">
      <div
        v-for="option in navigationStore.options"
        :key="option.label"
        class="text-4xl font-semibold first:mt-0 my-2"
      >
        <span
          v-if="option.icon"
          :class="`i-{{ option.icon }}`"
        ...
        <base-btn
          :color="option.color"
          @click="runEvent({action: option.action, url: option.to});"
        >
          {{ option.label }}
        </base-btn>
      </div>
