...
const patchProject = async (): Promise<void> => {
	if (!props.project) {
	...
	<BaseCard>
		...
		<div class="form-control">
			...
			<span class="text-sm text-slate-600"
				>unchecked project are only visible by website admin</span
			>
		</div>
		...
		<div class="flex flex-row gap-2 pt-2" style="border-top: 3px solid">
			...
			<base-btn
				...
				>Update</base-btn
			>
			<base-btn v-if="props.project" class="bg-red-400" @click="deleteProject()"
				>Delete</base-btn
			>
		</div>
	</BaseCard>
